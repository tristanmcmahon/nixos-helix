...
printf 'Helix installation storage check (read only)\nUEFI boot: %s\n\n' \
  "$([[ -d /sys/firmware/efi ]] && printf yes || printf no)"
...
printf 'Disk bytes: %s; partition bytes: %s; approximate unallocated bytes: %s\n' \
  "$disk_bytes" "$partition_bytes" "$((disk_bytes - partition_bytes))"
