...
if [[ ! -r $HELIX_ROOT_NIXOS_CHANNEL/default.nix ]]; then
  printf 'ERROR: root NixOS channel is unavailable at %s.\n' \
    "$HELIX_ROOT_NIXOS_CHANNEL" >&2
  printf 'Restore or select the root channel before checking or building Helix.\n' >&2
...
export NIX_PATH="nixpkgs=$HELIX_ROOT_NIXOS_CHANNEL:nixos-config=$HELIX_REPO_ROOT/configuration.nix:$HELIX_ROOT_NIXOS_CHANNEL"
HELIX_SELECTED_RELEASE=$(
  nix-instantiate --eval --raw -E '(import <nixpkgs> {}).lib.trivial.release'
)
export HELIX_REPO_ROOT HELIX_ROOT_NIXOS_CHANNEL HELIX_EXPECTED_RELEASE HELIX_SELECTED_RELEASE
